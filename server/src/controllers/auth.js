...
exports.login = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).cookie('token', token, { httpOnly: true, maxAge: 1000 * 60 * 5 }).json({
            success: true,
...
exports.logout = async (req, res) => {
    try {
        return res.status(200).clearCookie('token', { httpOnly: true, maxAge: 1000 * 60 * 5 }).json({
            success: true,
